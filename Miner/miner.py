...
class Miner():
    def __init__(self, project_path, log_path, **kwargs):
        ...
        self.cost_types = kwargs.get('cost_types') or []
    ...
    def get_times_and_transmission_times(self):
        ...
        with open(self.project_path + 'generated_times.cfg', 'w+') as f: 
            f.write('{\n')
            for key, value in soj_time.items():
                f.write(f'{key}: ({value}, {value/10})\n')
            f.write('}')
            f.close()
        with open(self.project_path + 'generated_tranmission_times.cfg', 'w+') as f: 
            f.write('{\n')
